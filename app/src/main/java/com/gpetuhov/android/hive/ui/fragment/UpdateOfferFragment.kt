...
class UpdateOfferFragment : BaseFragment(), UpdateOfferFragmentView {
    ...
    override fun openReviews(userUid: String, offerUid: String) {
        val action = UpdateOfferFragmentDirections.actionUpdateOfferFragmentToReviewsFragment(userUid, offerUid, true)
        findNavController().navigate(action)
    }
    ...
    private fun initQuitDialog() {
        if (context != null) {
            quitDialog = MaterialDialog(context!!)
                ...
                .neutralButton(R.string.cancel) { presenter.quitOfferUpdateCancel() }
        }
    }
